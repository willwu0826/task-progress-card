...
- **当前任务：**阶段箭头图串起整体框架，点击阶段展开步骤、工作类目、交付物和验收条件；当前位置、返修与返回点单独呈现。
- **本对话记录：**长文先显示章节分类和摘要，只展开选中的章节，事件按日期折叠；保留原文入口，不把当前文件冒充历史快照。
- **兼容旧卡：**阶段信息为可选字段；缺失或分组不完整时显示完整步骤清单，不自动补造进度。

配置见[可选阶段图与工作细目](references/operations.md#可选阶段图与工作细目)，版本变化见 [CHANGELOG.md](CHANGELOG.md)。本版没有开机自启、固定按钮或全对话自动接入；自动化回归与真实浏览器视觉验收的区别见[发布范围](references/scope.md)。
...
页面依赖本地服务。关闭 Codex 或重启电脑后，页面地址可能仍保留，但服务已停止；这不代表卡片丢失。本版不安装开机自启。让 Codex 复用原数据、启动对应服务，再刷新已有标签；不必重建任务卡。启动前应确认端口没有被其他程序占用。
